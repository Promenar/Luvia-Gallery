...
- **性能与安全优化 (v2.1.1)**:
    - **后端配置缓存**: 在 `server.js` 中引入了内存缓存机制，将 `lumina-config.json` 的磁盘 I/O 降至最低，大幅提升 API 响应速度。
    - **前端性能加固**: 在 `App.tsx` 中实现了 `persistData` 的防抖（Debounce）处理，解决了修改网站标题等配置时因频繁同步导致的 UI 卡顿。
    - **Tailwind 预编译**: 彻底移除了 `index.html` 中的 Tailwind CDN，通过 Vite 预编译 CSS 提升加载速度并解决生产环境下的 CDN 警告。
    - **正则用户缩略图修复**: 彻底修复了普通用户在 `checkFileAccess` 判定中无法通过 `allowedPaths` 校验的 Bug，确保权限受限用户也能正常查看授权目录的缩略图。
