...
- **模块边界**：`:app` 负责启动与认证态分流；`:core:model` 定义不可变会话模型；`:core:network`
  ...
  ViewModel 和登录界面。
- **已实现主链路**：受控服务器地址校验、唯一 Bearer 认证头、Retrofit 协程 `suspend` 登录请求与结果映射，
  以及 Material 3 登录页。登录成功后进入明确标识为“原生重构进行中”的主壳，用户可退出登录。
- **安全边界**：只接受受控 HTTPS 地址；网络层不将响应体或异常敏感内容暴露给 UI；协程取消继续传播。
...
原生图库、文件夹、收藏、媒体播放、设置、管理、加密持久会话、完整导航及端到端 UI 验证均留待后续阶段。
